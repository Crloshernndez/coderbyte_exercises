...
integers ranging from 1...n and the letter "E" and return the correct subset based on the following rules.
...
return that new set with integers separated by commas. For example:
...
def OffLineMinimum(strArr):
  ...
  for i in range(0, len(strArr)):
    elem = strArr[i]
    if elem == 'E':
      a = min(set(strArr[0:i]).difference(set(result)))
      result.append(a)
  return result
